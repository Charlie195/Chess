...
public class Gameboard {
    ...
    // Array of 8x8 board
    public Piece[][] selectedBoard = new Piece[8][8];
    ...
    public Gameboard() {
        ...
        for (int i = 0; i < selectedBoard.length; i++) {
            for (int j = 0; j < selectedBoard.length; j++) {
                if ((i + j) % 2 == 0) {
                    selectedBoard[i][j] = new Piece(i, j, 0, selectedBoard, whiteImg);
                } else {
                    selectedBoard[i][j] = new Piece(i, j, 0, selectedBoard, blackImg);
                }
            }
        }
    }
    ...
    public void print() {
        ...
        for (int y = 0; y < 8; y++) {
            for (int x = 0; x < 8; x++) {
                ...
                line += (selectedBoard[x][y].getClass().toString() + "\t").replaceAll("class Pieces.", "");
            }
...
//    public void copyBoard(Gameboard another) {
